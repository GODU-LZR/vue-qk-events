      ...
      <el-table :data="group.events" style="width: 100%">
        ...
        <el-table-column align="right">
          <template slot-scope="scope">
            ...
            <el-button
                size="mini"
                :type="scope.row.is_followed ? 'danger' : 'primary'"
                @click="handleFollow(scope.row)">
              {{ scope.row.is_followed ? '取关' : '关注' }}
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 选中比赛后,申请更换页面，查看比赛详情
    toMatch(matchId) {
      // 向父组件发送消息,请求更换路由页面,并将比赛id作为参数
      this.$emit("to-match", matchId);
    },

    /* 前端函数 */
    handleFollow(row) {
      // 向父组件发送消息,请求关注或取消关注赛事,并将比赛id作为参数
      this.$emit("handle-follow", row);
    },
